...
export function strapiLoader({
    ...
    return {
        ...
        load: async (context: LoaderContext) => {
            const strapiUrl = url || import.meta.env.STRAPI_URL || 'http://localhost:1337';
            ...
            try {
                do {
                    ...
                    if (Array.isArray(data.data)) {
                        ...
                        if (typeof data === 'object') {
                            ...
                            context.logger.warn(`Unexpected data structure for ${contentType}`);
                        }
                    }
                ...
                context.logger.info(`Loaded ${allEntries.length} entries for ${contentType}`);
                ...
                allEntries.forEach((entry: any) => {
                    ...
                    const id = entry.id?.toString() || entry.documentId?.toString() || data.slug;

                    if (!id) {
                        context.logger.warn(`Entry for ${contentType} missing ID, skipping.`);
                        return;
                    }
                ...
                context.logger.error(`Error loading ${contentType}: ${(error as Error).message}`);
                throw error;
            }
        },
    };
